  ...
  def self.up
    execute <<-EOS
      ...
      SELECT issues.id AS issue_id,
             issues.assigned_to_id AS user_id,
             COALESCE(issues.start_date, issues.due_date) AS start_date,
      ...
      GROUP BY issues.id, issues.assigned_to_id, issues.start_date, issues.due_date;
    EOS
  end
